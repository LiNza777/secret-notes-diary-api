...
- AI-Саммаризация (LLM): Асинхронная генерация кратких выжимок для заметок через OpenRouter API. Реализована отказоустойчивость с механизмом retry и обработкой таймаутов сторонних сервисов.
- Единый логгер по всему проекту (logging вместо print) с уровнями INFO/WARNING/ERROR
- fail-closed обработка сбоя Redis (503, без утечки внутренних деталей в проде), централизованные exception handlers для всего проекта
...
└── services_llm.py       # Интеграция с внешним AI-сервисом (OpenRouter) и retry-логика
```
...
1. Клонируйте репозиторий:
   ...
   cd secret-notes-api
   ```
2. Создайте файл `.env` на основе шаблона и заполните его своими данными(включая OPENROUTER_API_KEY для работы AI):
   ```bash
   cp .env.example .env
   ```
...
- `notes` (id, title, content, created_at, ai_summary, updated_at, owner_id → users.id)
...
- `POST /notes/{id}/summary` — сгенерировать ИИ-выжимку для конкретной заметки
...
4. Укажите параметры нагрузки и локальный URL API http://localhost:8000 или [ссылку на Render]([(https://secret-notes-diary-api.onrender.com/docs)], чтобы запустить тест.
